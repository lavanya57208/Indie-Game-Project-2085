...
export class GameEngine {
  ...
  private updateChase(dt: number) {
    ...
    // During chase
    if (this.chaseActive) {
      const result = this.alien.update(dt, this.player.z, this.player.speed, this.energy);
      ...
      // Barrier blocks alien
      if (this.barrierActive) {
        this.alien.group.position.z = Math.max(this.alien.group.position.z, this.player.z + 12);
      }
